...
public class ApiDelegateTest {
    ...
    public void sendMessage() {
        ...
        boolean status = apiSendMessageDelegate.sendText(myappid, Arrays.asList(user, "no_this_user"), null,
                agentIdConfig.getJava(myappid), "test now:" + DateUtils.dateToString(new Date()));
        logger.info("sendText status:" + status);

        status = apiSendMessageDelegate.sendNews(myappid, Arrays.asList(user), null, agentIdConfig.getJava(myappid),
